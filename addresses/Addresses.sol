...
contract Addresses is IAddresses, Test {
    ...
    constructor(string memory addressesFolderPath, uint256[] memory chainIds) {
        for (uint256 i; i < chainIds.length; ++i) {
            string memory addressesPath =
                string(abi.encodePacked(addressesFolderPath, "/", vm.toString(chainIds[i]), ".json"));
            ...
            for (uint256 j = 0; j < savedAddresses.length; j++) {
                _addAddress(savedAddresses[j].name, savedAddresses[j].addr, chainIds[i], savedAddresses[j].isContract);
            }
        }
    }
    ...
    function changeAddress(string memory name, address _addr, uint256 chainId, bool isContract) public {
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Address: ", name, " doesn't exist on chain: ", vm.toString(chainId), ". Use addAddress instead"
                )
            )
        );
        ...
        require(
            data.addr != _addr,
            string(
                abi.encodePacked("Address: ", name, " already set to the same value on chain: ", vm.toString(chainId))
            )
        );
    ...
    function printJSONChanges() external view {
        {
            ...
            if (names.length > 0) {
                ...
                for (uint256 j = 0; j < names.length; j++) {
                    ...
                    console.log("        'name': '%s'\n}%s", names[j], j < names.length - 1 ? "," : "");
                }
            }
        }

        {
            (string[] memory names,,, address[] memory addresses) = getChangedAddresses();
    ...
    function _addAddress(string memory name, address addr, uint256 chainId, bool isContract) private {
        ...
        require(
            !exist,
            string(
                abi.encodePacked("Address: ", addressToString(addr), " already set on chain: ", vm.toString(chainId))
            )
        );
    ...
    function _getAddress(string memory name, uint256 chainId) private view returns (address addr) {
        ...
        require(
            addr != address(0), string(abi.encodePacked("Address: ", name, " not set on chain: ", vm.toString(chainId)))
        );
    }
    ...
    function _checkAddress(address _addr, bool isContract, string memory name, uint256 chainId) private view {
        if (chainId == block.chainid) {
            if (isContract) {
                require(
                    _addr.code.length > 0,
                    string(abi.encodePacked("Address: ", name, " is not a contract on chain: ", vm.toString(chainId)))
                );
            } else {
                require(
                    _addr.code.length == 0,
                    string(abi.encodePacked("Address: ", name, " is a contract on chain: ", vm.toString(chainId)))
                );
            }
        }
    }
